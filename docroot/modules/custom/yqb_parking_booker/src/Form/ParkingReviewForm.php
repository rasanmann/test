...
class ParkingReviewForm extends ParkingFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($this->verifyCurrentTransaction()) {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($purchase->was_successful()) {
      $this->store->set('transaction_processed', TRUE);
      ...
      $purchaseData = [
        ...
        'transaction_number' => $purchase->transaction()->number(),
      ];
      $this->store->set('purchase_data', $purchaseData);
      ...
      $this->store->set('booking_guid', $bookingGuid);

      $confirmation = $this->confirmAdvamBooking($purchaseData, $transaction, $bookingGuid, $transactionRef);
      if (!$confirmation) {
  ...
  protected function confirmAdvamBooking($purchaseData, $transaction, $bookingGuid, $transactionRef) {
    $moneris = new MonerisConnector();

    $this->store->set('processed_amount', $purchaseData['payment_value']);

    // Invert exp date posiition
    $cardExpiryDate = substr($purchaseData['card_expiry_date'], 2) . substr($purchaseData['card_expiry_date'], 0, 2);
    ...
    $confirmationData = [
      ...
      "authCode" => $purchaseData['auth_code'],
      "authMessage" => $purchaseData['auth_message'],
      "responseCode" => $purchaseData['response_code'],
      "transactionCode" => $purchaseData['transaction_code'],
      "transactionRef" => $transactionRef,
      "paymentReference" => $purchaseData['order_id'],
      "cardType" => $purchaseData['card_type'],
      "cardNumber" => $purchaseData['masked_pan'],
      "cardExpiryDate" => $cardExpiryDate,
      "paymentValue" => $purchaseData['payment_value'],
    ];
    $this->store->set('confirmation_booking_data', $confirmationData);

    $confirmation = $this->advam->confirmBooking($confirmationData);
    if ($confirmation === FALSE) {
      // Void transaction immediately
      $void = $moneris->void($purchaseData['transaction_number']);
  ...
  protected function verifyCurrentTransaction() {
    if ($this->store->get('transaction_processed')) {
      $purchaseData = $this->store->get('purchase_data');
      $transaction = $this->store->get('transaction');
      $bookingGuid = $this->store->get('booking_guid');
      $transactionRef = $this->store->get('reference_number');
      $confirmation = $this->store->get('confirmation');
      ...
      if ($purchaseData && $transaction && $bookingGuid && $transactionRef) {
        if ($this->confirmAdvamBooking($purchaseData, $transaction, $bookingGuid, $transactionRef)) {
          return true;
        };
      }
    }
