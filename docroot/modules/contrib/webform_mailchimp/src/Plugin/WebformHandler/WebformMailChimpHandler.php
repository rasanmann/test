...
class WebformMailChimpHandler extends WebformHandlerBase {
  ...
  public function postSave(WebformSubmissionInterface $webform_submission, $update = TRUE) {
    ...
    \Drupal::moduleHandler()->alter('webform_mailchimp_lists_mergevars', $mergevars, $webform_submission, $this);
     ...
     // Interest groups
     $this->addInterestGroups($configuration, $webform_submission);
   ...
   */
  protected function addInterestGroups(&$configuration, $webform_submission) {
    $list = mailchimp_get_list($configuration['list']);
    if (!empty($list->intgroups)) {
      $fields = $this->getWebform()->getElementsDecodedAndFlattened();
      $fields_value = $webform_submission->toArray(TRUE);
      $data = !empty($fields_value["data"]) ? $fields_value["data"] : [];
      $conf_interest_groups = !empty($configuration["interest_groups"]) ?
        $configuration["interest_groups"] : [];

      foreach ($data as $field_name => $field_data) {
        $options = !empty($fields[$field_name]) && !empty($fields[$field_name]['#options']) ?
          $fields[$field_name]['#options'] : '';
        if (is_string($options) && strpos($options, WEBFORM_MAILCHIMP_ID_PREFIX) !== FALSE) {
          $group_id = str_replace(WEBFORM_MAILCHIMP_ID_PREFIX, '', $options);

          if (empty($conf_interest_groups[$group_id])) {
            $configuration["interest_groups"][$group_id] = [];
          }

          if (is_string($field_data)) {
            if (!empty($field_data) && empty($conf_interest_groups[$group_id][$field_data])) {
              $configuration["interest_groups"][$group_id][$field_data] = $field_data;
            }
          }
          elseif (is_array($field_data)) {
            foreach ($field_data as $id) {
              if (!empty($id) && empty($conf_interest_groups[$group_id][$id])) {
                $configuration["interest_groups"][$group_id][$id] = $id;
              }
            }
          }
        }
